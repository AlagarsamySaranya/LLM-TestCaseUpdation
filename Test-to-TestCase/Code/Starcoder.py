...
for _, row in df_train.iterrows():
    ...
    # Format input-output as prompt-completion pair
    train_data.append({
        "text": f"<s>Old Method:\n{old_method}\n\nNew Method:\n{new_method}\n\nOld Test Case:\n{old_test_case}\n\nGenerate New Test Case:</s> {new_test_case}"
    })
...
# Train model
print("Starting fine-tuning of StarCoder...")
trainer.train()
print("Fine-tuning complete.")

# Save the fine-tuned model
trainer.save_model("./starcoder_finetuned")
print("Model saved.")

# Load fine-tuned model for inference
fine_tuned_model = AutoModelForCausalLM.from_pretrained("./starcoder_finetuned")
fine_tuned_model.to("cuda")

# Function to generate new test cases
def generate_test_case(old_method, new_method, old_test_case):
    input_text = f"<s>Old Method:\n{old_method}\n\nNew Method:\n{new_method}\n\nOld Test Case:\n{old_test_case}\n\nGenerate New Test Case:</s>"
    inputs = tokenizer(input_text, return_tensors="pt").to("cuda")
    output = fine_tuned_model.generate(**inputs, max_length=300)
